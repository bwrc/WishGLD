    ...
    for cardColor in range(N_CLRS):
        ...
        for featN in range( N_OF_FEATS ):
            ...
            for featOrientation in range( 4 ):
                ...
                # Write params for one card
                outarr.append(str(DIMX) +' '+ str(DIMY) +' '+ str(IMG_W) +' '+ str(IMG_H) +' '+\
                            str(feats[featN]) +' '+ colors[cardColor] +' '+ str(45+90*featOrientation))

    # OPEN A CSV WRITING FILE
    with open('cardparams.csv','wb') as csvfile:
        pamwriter = csv.writer(csvfile, delimiter=' ', quotechar='|', quoting=csv.QUOTE_MINIMAL)
        for i in range(N_CLRS*N_OF_FEATS*4):
            pamwriter.writerow(outarr[i])
